...
def load_data(config):
    ...
    table = pd.read_csv(data_config['path'], sep=";", header=0, skipinitialspace=True)

    # set column names, quasi_id
    table.columns = data_config['columns']
    quasi_id = data_config['samarati_quasi_id']
    
    # Nur die ersten 4 Zeichen der Spalte 'BIRTHDATE' extrahieren und in int umwandeln
    table['BIRTHDATE'] = table['BIRTHDATE'].astype(str).str.slice(0, 4).astype(int)
#    table['DEATHDATE'] = table['DEATHDATE'].astype(str).str.slice(0, 4).replace('nan', None).fillna(0).astype(int)
...
def build_categorical_hierarchy(path):
    hierarchy = pd.read_csv(path, sep=",", header=None)
