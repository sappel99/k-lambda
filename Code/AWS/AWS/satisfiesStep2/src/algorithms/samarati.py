...
def samarati(table, lattice, leaves_num, sensitive, k=10, maxsup=20, optimal=False):
    ...
    if optimal:  # consider all possible vectors to get optimal solution
        ...
        all_impossible_solutions = []
        vectors = []
        for h in range(lattice.total_height):
            for v in lattice.get_vectors(h):
                vectors.append(v)
        #result = [{"vector": v, "k": k, "table": table, "maxsup": maxsup, "lattice": lattice} for v in vectors]
        for v in vectors:
            ...
            #                                    'time': elapsed_time})
            #else:
            #    all_impossible_solutions.append({'table': anonymized_table,
            #                                   'sup': sup,
            #                                   'vector': v,
            #                                   'loss_metric': categorical_loss_metric(
            #                                       anonymized_table.loc[:, lattice.quasi_id], 
        ...
        for sol in all_possible_solutions:
            print("Vector:", sol['vector'], "Loss Metric:", sol['loss_metric'], "Sup:", sol['sup'])
        #print('\nnumber of all impossible solutions:', len(all_impossible_solutions))
